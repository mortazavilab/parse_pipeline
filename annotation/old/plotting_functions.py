...
def ncells_gen_celltype(adata):
    median_umis = []
    median_genes = []

    for ct in adata.obs['general_celltype'].unique().tolist():
        adata_subset = adata[adata.obs['general_celltype'] == ct]
        median_umis.append(adata_subset.obs['total_counts'].median())
        median_genes.append(adata_subset.obs['n_genes_by_counts'].median())

    df = pd.DataFrame({'median_umis': median_umis, 'median_genes': median_genes, 'general_celltype': adata.obs['general_celltype'].unique()})

    df2 = pd.DataFrame(adata.obs['general_celltype'].value_counts()).reset_index()
    df2.columns = ['general_celltype', 'ncells']

    out = pd.merge(df, df2, on='general_celltype', how='left')

    return out

def ncells_celltype(adata):
    median_umis = []
    median_genes = []

    for ct in adata.obs['celltype'].unique().tolist():
        adata_subset = adata[adata.obs['celltype'] == ct]
        median_umis.append(adata_subset.obs['total_counts'].median())
        median_genes.append(adata_subset.obs['n_genes_by_counts'].median())

    df = pd.DataFrame({'median_umis': median_umis, 'median_genes': median_genes, 'celltype': adata.obs['celltype'].unique()})

    df2 = pd.DataFrame(adata.obs['celltype'].value_counts()).reset_index()
    df2.columns = ['celltype', 'ncells']

    out = pd.merge(df, df2, on='celltype', how='left')
